...
class SpUtils {
  ...
  ///保存视频到本地
  static Future<bool> saveVideo(String url) async {
    bool isSuccess = false;
    final Directory? downloadsDir = await getApplicationDocumentsDirectory();
    Dio dio = Dio();
    if (downloadsDir != null) {
      Response response =
      await dio.download(url, "${downloadsDir.path}/vip_video.mp4");
  ...
  ///判断某个文件是否存在(存在就返回地址否则返回null)
  static Future<String?> fileExit(String fileName) async {
    if (fileName.isNotEmpty) {
      final Directory? downloadsDir = await getApplicationDocumentsDirectory();
      if (downloadsDir != null) {
        if (await File("${downloadsDir.path}/$fileName").exists())
          return File("${downloadsDir.path}/$fileName").path;
      }
    }
    return null;
  }
